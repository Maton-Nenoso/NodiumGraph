...
**Round-trip contract** — two directions; **both scoped to `width > 0 && height > 0`** (consistent with `InferAnchor`'s zero-dimension throw in the zero-dimension contract):

1. **Anchor → point → anchor.** For any well-formed `PortAnchor a` and any `(w, h)` with `w > 0 && h > 0`, `InferAnchor(GetEdgePoint(a, w, h), w, h)` returns `a` exactly when `a` is canonical, and returns the canonical anchor for the same boundary point when `a` is not (i.e. when `a` is a `Fraction = 1` anchor on an edge whose endpoint is shared with the next edge). In both cases the *boundary point* is preserved exactly.

2. **Boundary point → anchor → boundary point.** For any point `p` lying on the boundary of the shape at `(w, h)` with `w > 0 && h > 0`, `GetEdgePoint(InferAnchor(p, w, h), w, h) == p` within float epsilon. Within that scope this direction holds unconditionally and is what makes `DynamicPortProvider` predictable: a port created at a boundary hit point lands exactly where the user clicked, including on rounded corners.

At `w == 0 || h == 0`, `InferAnchor` throws and round-trip is undefined; `GetEdgePoint` still returns `(0, 0)` per the zero-dimension contract but no inverse is meaningful.
...
| User guide pages | **Update.** Remove `layoutAware`, `ILayoutAwarePortProvider`, `PortEmissionDirection` references. Switch port-construction examples, model-reference signatures, persistence guidance, and routing-side invalidation notes to anchor form in (at minimum): `1-tutorial/getting-started.md`, `2-how-to/custom-port-provider.md`, `2-how-to/custom-node-template.md`, `2-how-to/style-ports.md`, `2-how-to/persist-graph-state.md` (any persisted port records `Anchor.Edge` + `Anchor.Fraction`, not derived world-unit `Position` — applies to both fixed-provider declarations and dynamic-provider snapshots since `DynamicPortProvider` now creates anchored ports too; show both cases), `2-how-to/custom-router.md` (line 180: `AbsolutePosition` is invalidated on owner-node move **and** on `Width` / `Height` / `Shape` change), `3-reference/strategies.md`, `3-reference/model.md` (the `Port` constructor block at lines 100–101 currently documents the deleted point-based constructors), `3-reference/rendering-pipeline.md` (note that `Port.Position` is derived from `Port.Anchor`). Per the global sweep in done criteria, audit beyond this list for any other point-based wording. |
...
6. User guide sweep — no stale **port-positioning, port-construction, or port-persistence** wording remains anywhere under `docs/userguide/`. Specifically: zero references to `layoutAware`, `ILayoutAwarePortProvider`, `PortEmissionDirection`; no `new Port(...)` examples constructing a port with a `Point`; no persistence guidance storing derived world-unit port positions instead of anchors; no statements that `Port.AbsolutePosition` invalidates only on node move (must include `Width` / `Height` / `Shape` change). Other point-based APIs remain valid and out of scope for this sweep — `Node.X`/`Y`, `ViewportZoom` / viewport coordinates, router-output point lists, `IPortProvider.ResolvePort(Point)`, drag/drop world coordinates, etc. — they are unaffected. The pages enumerated in the migration row are the known set; the sweep catches port-positioning prose elsewhere.
