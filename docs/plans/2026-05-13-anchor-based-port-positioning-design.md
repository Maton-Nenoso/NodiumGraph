...
| User guide pages | **Update.** Remove `layoutAware`, `ILayoutAwarePortProvider`, `PortEmissionDirection` references. Switch port-construction examples, model-reference signatures, persistence guidance, and routing-side invalidation notes to anchor form in (at minimum): `1-tutorial/getting-started.md`, `2-how-to/custom-port-provider.md`, `2-how-to/custom-node-template.md`, `2-how-to/style-ports.md`, `2-how-to/persist-graph-state.md` (fixed-port persistence should record `Anchor.Edge` + `Anchor.Fraction`, not derived world-unit `Position`), `2-how-to/custom-router.md` (line 180: `AbsolutePosition` is invalidated on owner-node move **and** on `Width` / `Height` / `Shape` change), `3-reference/strategies.md`, `3-reference/model.md` (the `Port` constructor block at lines 100–101 currently documents the deleted point-based constructors), `3-reference/rendering-pipeline.md` (note that `Port.Position` is derived from `Port.Anchor`). Per the global sweep in done criteria, audit beyond this list for any other point-based wording. |
...
6. User guide sweep — no stale point-based wording remains anywhere under `docs/userguide/`. Specifically: zero references to `layoutAware`, `ILayoutAwarePortProvider`, `PortEmissionDirection`; no `new Port(...)` examples using a `Point`; no persistence guidance recording derived world-unit port positions instead of anchors; no statements that `Port.AbsolutePosition` invalidates only on node move (must include `Width` / `Height` / `Shape` change). The pages enumerated in the migration row above are the known set; the sweep catches anything else.
